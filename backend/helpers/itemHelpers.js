...
export const getNextSequentialNumber = async itemId => {
  ...
  try {
    ...
    logger.error(`Error in getNextSequentialNumber for itemId ${itemId}:`, error);
    throw error;
  }
};

// Generate Draft Filename
export const generateDraftFilename = (itemId, sequentialNumber = 1, originalFilename = 'image') => {
  logger.info(
...
export const handleFileUploadAndSequentialNumber = async (itemId, originalFilename) => {
  logger.info(`Handling file upload and sequential number for itemId: ${itemId}`);
  return await lock.acquire(itemId, async () => {
    const sequentialNumber = await getNextSequentialNumber(itemId);
    const filename = generateDraftFilename(itemId, sequentialNumber, originalFilename);
